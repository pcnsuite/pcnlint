...
function testDocument(docData) {
	...
	for (var s = doc.steps.length -1; s >= 0; s--) {
		...
		if (step.problems) {
			...
			for (var p = step.problems.length - 1; p >= 0; i--) {
				expect(step.problems[p]).to.be.an('object');

				expect(step.problems[p]).to.have.property('type')
				.that.is.a('string')
				.and.to.satisfy(stringMatch(['inconvenient', 'confusing', 'difficult', 'likely_to_fail']));

				expect(step.problems[p]).to.have.property('description')
				.that.is.a('string');
			};
		}
	};
